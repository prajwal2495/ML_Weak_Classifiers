    ...
    def evaluate(self, decision):
        ...
        if decision not in self.evaluated:
            ...
            for fold in range(self.crossval_fold):
                ...
                test_set_size = int(np.ceil(self.data_X.shape[0]* 0.2))
                ...
                if type(objs_crossval) == type(None):
                    product_data_and_objs = (objs * len(self.data_y))
                    objs_crossval = product_data_and_objs
                else:
                    mean_of_all_objs_over_test_data = sum(objs) / self.crossval_fold #np.mean(objs) 
                    objs_crossval += mean_of_all_objs_over_test_data
    ...
    def crossover(self):
        ...
        def cross(a, b):
            ...
            for i in range(len(a)):
                randomPointSelection = np.random.randint(0, 2)
                if randomPointSelection == 0:
